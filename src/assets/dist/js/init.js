...
var mintos = function() {
  ...
  $(document).on("click", "#navtop_light_select,#navtop_dark_select", function(
    e
  ) {
    $(".hk-navbar-select")
      .find(".btn")
  ...
  $(document).on("click", "#navtop_light_select", function(e) {
    $navbar
      ...
      .find("img.brand-img")
      .attr("src", "dist/img/logo-light.png");
    return false;
  });
  $(document).on("click", "#navtop_dark_select", function(e) {
    $navbar
      ...
      .find("img.brand-img")
      .attr("src", "dist/img/logo-dark.png");
    return false;
  });
  ...
  $(document).on("click", "#reset_settings", function(e) {
    if (navBarChk) $("#navtop_dark_select").click();
    else $("#navtop_light_select").click();
    if (navChk) $("#nav_dark_select").click();
    else $("#nav_light_select").click();
  ...
  $(document).on("click", refreshMe, function(e) {
    var panelToRefresh = $(this)
      .closest(".card")
      .find(".refresh-container");
    var dataToRefresh = $(this)
      .closest(".card")
      .find(".panel-wrapper");
...
var setHeightWidth = function() {
  ...
  if ($vertnaltNav.hasClass("navbar-search-toggle"))
    $vertnaltNav
      .find(".chatapp-users-list,.chat-body")
      .css("height", height - 240);
  else
    $vertnaltNav
      .find(".chatapp-users-list,.chat-body")
      .css("height", height - 190);
  ...
  if ($vertnaltNav.hasClass("navbar-search-toggle"))
    $vertnaltNav
      .find(".calendarapp-sidebar,.calendar-wrap")
      .css("height", height - 107);
  else
    $vertnaltNav
      .find(".calendarapp-sidebar,.calendar-wrap")
      .css("height", height - 57);

  /*GmapApp Height for differnt brekpoints with Vertical & Alt menu*/
  if ($vertnaltNav.hasClass("navbar-search-toggle"))
    $vertnaltNav.find(".gmap").css("height", height - 107);
  else $vertnaltNav.find(".gmap").css("height", height - 57);
  ...
  if (width > 1024) {
    if (
      $horizontalNav.hasClass("hk-nav-toggle") &&
      !$horizontalNav.hasClass("navbar-search-toggle")
    )
      $horizontalNav
        .find(".chatapp-users-list,.chat-body")
        .css("height", height - 190);
  ...
  if (width > 1024) {
    ...
    else $horizontalNav.find(".gmap").css("height", height - 107);
  } else {
    if ($horizontalNav.hasClass("navbar-search-toggle"))
      $horizontalNav.find(".gmap").css("height", height - 107);
    else $horizontalNav.find(".gmap").css("height", height - 57);
  }
};
...
var chatApp = function() {
  if (width > 1024) chatAppTarget.removeClass("chatapp-slide");
  $(document).on("click", ".chatapp-wrap .chatapp-users-list a.media", function(
    e
  ) {
    if (width <= 1024) {
    ...
    if (e.which == 13 && !$(this).val().length == 0) {
      $(
        '<li class="media sent"><div class="media-body"><div class="msg-box"><div><p>' +
          $(this).val() +
          '</p><span class="chat-time">8:00 pm</span><div class="arrow-triangle-wrap"><div class="arrow-triangle left"></div></div></div></div></div></li>'
...
var emailApp = function() {
  if (width > 1024) emailAppTarget.removeClass("emailapp-slide");
  $(document).on(
    "click",
    ".emailapp-wrap .emailapp-emails-list a.media",
    function(e) {
      if (width <= 1024) {
