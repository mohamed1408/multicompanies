...
import { dtrangepicker } from 'src/assets/dist/js/datePickerHelper';
declare function setHeightWidth(): any;

@Component({
  selector: 'app-weborder-rpt',
  templateUrl: './weborder-rpt.component.html',
  styleUrls: ['./weborder-rpt.component.css'],
})
export class WeborderRptComponent implements OnInit {
  storeid: number = 0;
  sourceid: number = 0;
  companyid: number = 0;
  ...
  selectOrder(odrsid: number, e: Event) {
    ...
    this.selectedOrder.transaxns = this.transaxns
      .filter((x: any) => x.OdrsId == odrsid)
      .map((x) => {
    ...
    setTimeout(() => {
      let top_offset =
        (document.querySelectorAll('section.sticky-top')[0] as HTMLElement)
          .offsetHeight + 14;
      if (document.querySelectorAll('section.sticky-top')[1])
        (
          document.querySelectorAll('section.sticky-top')[1] as HTMLElement
        ).style.top = `calc(${top_offset}px + 220px)`;
    }, 500);
  }
  clearselection() {
